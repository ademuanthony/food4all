        ...
        <div class="col s12">
            <table class="table bordered">
                ...
                foreach($page_info->data as $product){
                    ...
                    <tr>
                        <th scope="row"><?= (++$no) ?></th>
                        <td><img style="max-width: 100px; max-height: 100px;" class="img img-thumbnail"
                                 src="<?= $view['assets']->getUrl($product->getFileFullName()) ?>"></td>
                        <td><?= $product->getName() ?></td>
                        <td><?= $product->getDescription() ?></td>
                        <td><?= $product->getOldPrice() ?></td>
                        <td><?= $product->getNewPrice() ?></td>
                        <td>
                            <a class="btn btn-danger" href="<?= TinyMvc::toRoute(AppService::RouteBackendManageProduct, ['id' => $product->getId()]) ?>">Edit</a>
                            <a class="btn btn-danger" href="<?= TinyMvc::toRoute(AppService::RouteBackendDeleteProduct, ['id' => $product->getId()]) ?>">Delete</a>
                            <a target="_blank" class="btn btn-danger" href="<?= TinyMvc::toRoute(AppService::RouteFrontendViewProduct, ['permalink' => $product->getPermalink()]) ?>">View</a>
                        </td>
                    </tr>
                    <?php
                }
...
<div class="fixed-action-btn" style="bottom: 50px; right: 19px;">
    ...
    <ul>
        <li><a title="Add Category" href="<?= TinyMvc::toRoute(AppService::RouteBackendAddProduct) ?>" class="btn-floating red"><i class="large mdi-content-add"></i></a></li>
    </ul>
</div>
<!-- Floating Action Button -->
